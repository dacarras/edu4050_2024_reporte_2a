...
- De los datos disponibles, elija un pais para realizar el siguiente ejercicio:
...
- Recodifique las respuestas de los items del test. En la siguiente tabla se incluye las claves de respuestas correctas de cada item, en `[k = *]`
...
- Ajuste un modelo Rasch sobre los 16 items del test.
...
- Despliegue los resultados obtenidos empleando a `summary()`
...
- Extraiga los puntajes IRT de tipo WLE, empleando a la functión `TAM::tam.wle()`, y calcule la media obtenida de los puntajes generados.
...
- Genere un histograma de los puntajes IRT generados.

```{r}

# emple la función hist(), u otra equivalente.

```

# Ejercicio 8

- Obtenga una tabla de los delta estimados, y luego:
  + Ordene a los delta obtenidos de menor a mayor
  + Finalmente, indique si los delta producidos generan el mismo orden del ejemplo.



```{r}

# Siga el siguiente código como ejemplo
...
- Ajuste el mismo modelo para otro pais, extraiga los theta wle, y calcule la media de estos puntajes.
